...
function WcTenure({ setRoute, months, setMonths, nextRoute }: { setRoute: (r: RouteKey) => void; months: number; setMonths: (m: number) => void; nextRoute: RouteKey }) {
  ...
  const benefitLine = wcDiscountAmountFor(months) > 0
    ? <>You save <Riyal size={12} color={greenMid} /> {formatAmount(Math.round(wcDiscountAmountFor(months)))} with {wcDiscountPctFor(months)}% off</>
              ...
              <Animated.View pointerEvents="box-none" style={[styles.wcBenefitWrap, { transform: [{ translateY: rise.interpolate({ inputRange: [0, 1], outputRange: [280, 0] }) }] }]}>
                {cartVariant === 'dock' ? (
                  ...
                  >
                    <View style={styles.wcSheetCartLeft}>
                      <Image source={figmaImageSource('wcCartIcon')} resizeMode="contain" accessibilityIgnoresInvertColors style={{ width: 15, height: 15 }} />
                      <Text style={styles.wcDockItems}>{WC_CART_ITEMS} {WC_CART_ITEMS === 1 ? 'Item' : 'Items'}</Text>
                    </View>
                    <View style={styles.wcSheetCartRight}>
